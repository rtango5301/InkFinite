...
interface CustomToolbarProps {
	onNotify?: (n: { message: string; type: 'success' | 'processing' } | null) => void
}
...
export function CustomToolbar({ onNotify }: CustomToolbarProps) {
	...
	const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
		...
		if (file) {
			...
			reader.onerror = () => {
				onNotify?.({ message: 'Failed to read image file', type: 'processing' })
				e.target.value = ''
			}
	...
	const exportAsImage = async () => {
		onNotify?.({ message: 'Exporting PNG…', type: 'processing' })
		try {
			...
			if (png) {
				...
				if (blob) {
					...
					triggerDownload(url, 'infinite-canvas-export.png')
					onNotify?.({ message: 'PNG exported!', type: 'success' })
					return
				}
			}
			onNotify?.(null)
	...
	const exportAsJSON = () => {
		onNotify?.({ message: 'Saving JSON…', type: 'processing' })
		try {
			const snapshot = editor.getSnapshot()
			const json = JSON.stringify(snapshot, null, 2)
			const blob = new Blob([json], { type: 'application/json' })
			const url = URL.createObjectURL(blob)
			triggerDownload(url, 'infinite-canvas-backup.json')
			onNotify?.({ message: 'JSON saved!', type: 'success' })
		} catch (e) {
